...
public class Level : MonoBehaviour
{
    ...
    private void SpawnInitialClouds() {
        ...
        cloudTransform = Instantiate(GetCloudPrefabTransform(), new Vector3(-92f, CLOUD_SPAWN_Y_POSITION, 0), Quaternion.identity);
        cloudList.Add(cloudTransform);
        cloudTransform = Instantiate(GetCloudPrefabTransform(), new Vector3(34f, CLOUD_SPAWN_Y_POSITION, 0), Quaternion.identity);
        cloudList.Add(cloudTransform);
    }
    ...
    private void HandleClouds() {
        ...
        if (cloudSpawnTimer < 0) {
            ...
            Transform cloudTransform; 
            cloudTransform = Instantiate(GetCloudPrefabTransform(), new Vector3(CLOUD_SPAWN_X_POSITION, CLOUD_SPAWN_Y_POSITION, 0), Quaternion.identity);
            cloudList.Add(cloudTransform);
        }
    ...
    private void SpawnInitialGround() {
        ...
        groundTransform = Instantiate(GameAssets.GetInstance().pfGound, new Vector3(0, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
        groundTransform = Instantiate(GameAssets.GetInstance().pfGound, new Vector3(groundWidth, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
        groundTransform = Instantiate(GameAssets.GetInstance().pfGound, new Vector3(groundWidth * 2f, groundY, 0), Quaternion.identity);
        groundList.Add(groundTransform);
    }
    ...
    private void HandlePipeMovement() {
        for (int i=0; i<pipeList.Count; i++) {
            ...
            pipe.Move();
            if (isToTheRightOfBird && pipe.GetXPosition() <= BIRD_X_POSITION && pipe.IsBottom()) {             // Pipe passed Bird
                pipesPassedCount++;
    ...
    private void CreatePipe(float height, float xPosition, bool isBottom) {
        // Set up Pipe Head
        Transform pipeHead = Instantiate(GameAssets.GetInstance().pfPipeHead);                  // Get pfPipeHead prefab, set to pipeHead, and set its correct position
        ...
        // Set up Pipe Body
        Transform pipeBody = Instantiate(GameAssets.GetInstance().pfPipeBody);                  // Make pfPipeBody prefab, set to pipeBody, and set its correct position
        float pipeBodyYPosition;
        if (isBottom) {
            pipeBodyYPosition = -CAMERA_ORTHO_SIZE;
        } else {
            pipeBodyYPosition = +CAMERA_ORTHO_SIZE;
            pipeBody.localScale = new Vector3(1, -1, 1);                                        // Because pivot on sprite was set to bottom
        }        
        pipeBody.position = new Vector3(xPosition, pipeBodyYPosition);

        
        SpriteRenderer pipeBodySpriteRenderer = pipeBody.GetComponent<SpriteRenderer>();        // Resize pipeBody to be the correct passed in height
        pipeBodySpriteRenderer.size = new Vector2(PIPE_WIDTH, height);

        BoxCollider2D pipeBodyBoxCollider = pipeBody.GetComponent<BoxCollider2D>();             
        pipeBodyBoxCollider.size = new Vector2(PIPE_WIDTH, height);                             // Resize pipeBody's box collider to be the correct passed in height
        pipeBodyBoxCollider.offset = new Vector2(0f, height * 0.5f);                            // Offset pipeBody's box collider up because sprite pivot was set to bottom
